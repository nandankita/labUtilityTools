...
def saddleplot(
    ...
    """
    #expected = pd.read_csv(expected1, sep='\t',  names=['chrom', 'distance', 'avg'])
    
    
    track = pd.read_csv(track1,  sep='\t',  names=['chrom', 'start', 'end', 'eigen'])
    clr = cooler.Cooler(clr_file)
    if contact_type == 'cis':
        expected = pd.read_csv(expectedFile, sep=',',  names=['chrom','distance','n_valid','balanced.sum','balanced.avg'])
        ...
        with open(expectedFile) as expFH:
            p=expFH.readlines()
        e=p[1].split("\t")
        expected=float(e[2])
    ...
    df = pd.DataFrame(saddledata)
    df.to_csv("saddleData.txt", sep='\t')
    
    df2=pd.DataFrame(np.log10(saddledata))
    
    df2.to_csv("log10_saddleData.txt", sep='\t')
...
def get_arguments():
    ...
    parser.add_argument("-t" ,
                        metavar = 'track_file' ,
                        help = "bedGraph-like data frame ('chrom', 'start', 'end', <name>)" ,
                        dest = "track",
                        required = True ,
                        type = str)
    ...
    parser.add_argument("-n" ,
                        metavar = 'track_name' ,
                        help = "Name of value column in ``track``" ,
                        dest = "track_name",
                        required = True ,
                        type = str)

    parser.add_argument("-c" ,
                        metavar = 'contact_type' ,
                        help = "Type of contacts to aggregate(cis or trans). Default is cis.",
                        dest = "contact_type",
                        required = False ,
                        default = 'cis' ,
                        type = str)
    parser.add_argument("-e" ,
                        metavar = 'expected_file' ,
                        help = "expected file" ,
                        dest = "expected",
                        required = True ,
                        type = str)
...
def main():
    ...
    track_file = os.path.abspath(arguments.track)
    track_name = str(arguments.track_name)
    contact_type = str(arguments.contact_type)
    expected = os.path.abspath(arguments.expected)
    expected_name = str(arguments.expected_name)

    _, _, _, fig = saddleplot(input_file, track_file, track_name, contact_type, expected, expected_name)
    fileName=os.path.basename(input_file)
    o=str(fileName).split(".")
    outFile=o[0]+"."+contact_type+".saddle.plot.pdf"
